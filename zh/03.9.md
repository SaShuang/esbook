...
## 分析  
如果使用工作台填报某个模板,保存的记录,会带上es的一些标记,比如ExcelServerRCID, RN,如果存在工作流的话还会增加ExcelServerWIID等等,另外还会对系统的一些维护表进行写入数据.所以,如果直接用SQL的一条INSERT语句插入的数据只会出现在后台,不会出现在工作台的显示界面.
